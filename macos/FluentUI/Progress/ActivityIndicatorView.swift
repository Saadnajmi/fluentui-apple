...
/// defines the side size of the loader and the thickness of the loader stroke.
...
public extension Colors {
	struct ActivityIndicator {
		public static var foreground: NSColor = .systemBlue // TODO iconSecondary
	}
}
...
open class ActivityIndicatorView: NSView, ActivityViewAnimating {
    ...
    @objc open var color: NSColor = Colors.ActivityIndicator.foreground {
        didSet {
            setupLoaderLayer()
        }
    }
    ...
    private func setupLoaderLayer() {
		...
		let loaderPath = NSBezierPath()
		loaderPath.appendArc(withCenter: CGPoint(x: sideSize / 2.0, y: sideSize / 2.0), radius: strokeRadius, startAngle: angles.startAngle, endAngle: angles.endAngle, clockwise: true)
...
//        shapeLayer.contentsScale = UIScreen.main.scale TODO
